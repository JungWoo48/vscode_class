...
function fnTest1() {
    ...
    // id 가 "div1"인 요소를 얻어와 상수형 변수 div에 저장
    const div1 = document.getElementById("div1");

    // 접근한 요쇼의 배경색을 얻어와 변수에 저장
    const backColor = div1.style.backgroundColor;
...
function fnTest3() {
    // 문서 내에 존재하는 모든 li요소를 얻어와 배열 향식으로 반환
...
function fnTest4() {
    ...
    // input 관련 태그에 작성된 값을 나타낸느 속성 == value

    // input 요소에 작성된 값을 얻어와 alert 출력
    alert(input.value);

    // input 요소애 ""(빈 문자열) 대입 == 값을 삭제
    input.value = "";
}
...
function fnTest5() {
    ...
    // name 속성이 "hobby"인 사람 불러오기
    const hobby = document.getElementsByName("hobby");

    // console.log(hobby);

    let str = "";// 체크된 체크 박스의 값을 누적할 변수 선언
    ...
    if(str != "") { //체크가 된 경우
        ...
        result5.innerHTML += "<br><br> 선택한 개수 : " + count;
